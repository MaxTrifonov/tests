...
class EMailer
{
   ...
   public function getView($view, $vars = array(), $layout = null)
   {
       if (Yii::app() instanceof CConsoleApplication) {
           $this->getViewConsole($view, $vars, $layout);
           return $this->_myMailer->Body;
       }
    ...
    /**
     * Get view file if we run this from console
     *
     * @param string $view
     * @param array $vars
     * @param string $layout
    ...
    {
        $template = $this->pathViews . '.' . $view;
        $template = str_replace('.', DIRECTORY_SEPARATOR, $template);
        $template = str_replace('application', 'protected', $template);

        $body = CController::renderInternal($template . '.php', array_merge($vars, array('content' => $this->_myMailer)), true);
        if ($layout === null) {
            $this->_myMailer->Body = $body;
        }
        else {
            $layoutTemplate = $this->pathLayouts . '.' . $layout;
            $layoutTemplate = str_replace('.', DIRECTORY_SEPARATOR, $layoutTemplate);
            $layoutTemplate = str_replace('application', 'protected', $layoutTemplate);
            $this->_myMailer->Body = Yii::app()->controller->renderPartial($layoutTemplate, array('content' => $body), true);
        }
    }
     ...
     * Add recepients
     *
     * @param $address - array of emails or string
     * @param string $name
     * @return bool
     */
    public function AddAddress($address, $name = '')
    {
        ...
        if (is_array($address)) {
            foreach ($address as $to) {
                $result = $result && $this->_myMailer->AddAddress($to);
            }
        } else {
            $result = $result && $this->_myMailer->AddAddress($address, $name);
        }
     ...
     *
     * Clears recipients after sending mail
     *
     * @return bool
     */
    public function Send()
    {
        $result = $this->_myMailer->Send();
        
        $this->_myMailer->ClearAddresses();
        $this->_myMailer->ClearCCs();
        $this->_myMailer->ClearBCCs();
        $this->_myMailer->ClearReplyTos();
        $this->_myMailer->ClearAttachments();
        $this->_myMailer->ClearCustomHeaders();
